"""Run the **frozen v2 motion baseline** on the full clip.

This script is a deliberate snapshot of the motion-only classical
baseline as it stood at commit 3f3aed1, before the AND-gate /
Mahalanobis-gate / template-bank / two-state refinements were added.

Why keep it as a standalone reference:
    - It's the strongest *purely classical* (no DL) tracker the project
      built, and the cleanest visual baseline for the hybrid work.
    - Single target, one bbox, one persistence-heatmap panel — easiest
      to read at a glance.
    - No torch / ultralytics / DL weights required.

Usage:
    python scripts/run_motion_v2.py \
        --video assets/How_to_hide_from_a_thermal_drone_Ukraine.mp4 \
        --out outputs/motion_v2.mp4 --side-by-side
"""
...
from baseline_v2.pipeline import (MotionPipeline, MotionPipelineConfig,  # noqa: E402
                                Pipeline, PipelineConfig)
from baseline_v2.tracker import TrackState  # noqa: E402
from baseline_v2.visualize import (draw_candidates, draw_hud, draw_track,  # noqa: E402
